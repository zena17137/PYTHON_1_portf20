...
tk.title('Xрестики нолики')
tk.resizable(40, 40)
tk.wm_attributes('-topmost', 1)
canvas = Canvas(tk, width=s_canvas_x, height=s_canvas_y, bd=0, highlightthickness=0)
canvas.create_rectangle(0, 0, s_canvas_x, s_canvas_y, fill='white')
canvas.pack()
tk.update()

s_x = 3
s_y = 3
step_x = s_canvas_x // s_x
step_y = s_canvas_y // s_y
...
def draw_table():
	for i in range(0, s_x):
		canvas.create_line(0, i*step_y, s_canvas_x, i*step_y)
	for i in range(0, s_y):
		canvas.create_line(i*step_y, 0, i*step_x , s_canvas_x)
...
def add_to_points(event):
	...
	if points[event.x // step_x][event.y // step_y] == -1:
		...
		print('єєєєє чувак полегче тут уже стоит чтото')
...
b1 = Button(tk, text='Почати знову2', command=button_press)
b1.pack()
